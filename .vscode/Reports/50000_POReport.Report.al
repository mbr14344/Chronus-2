...
{
    RDLCLayout = './.vscode/ReportLayout/PurchaseOrderCustom.rdl';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            column(INNER_GVar; INNER_GVar)
            {

            }
            column(MPACK_GVar; MPACK_GVar)
            {

            }
            column(Lot_Desc; Lot_Desc)
            {
            ...
            column(DoNotShipBeforeDate; DoNotShipBeforeDate)
            {

            }
            column(RequestedCargoReadyDate; RequestedCargoReadyDate)
            {

            }
            column(RequestedInWhseDate; RequestedInWhseDate)
            {
            ...
            dataitem("Purchase Line"; "Purchase Line")
            {
                ...
                var
                    ...
                    No_Series_LRec: Record "No. Series";
                    RE: Record "Reservation Entry";
                    ITS: Record "Tracking Specification";
                ...
                begin
                    ...
                    MPACK_GVar := 0;
                    LotNo := '';
                    ItemUnitOfMasure_GRec.SetRange("Item No.", "Purchase Line"."No.");
                    ItemUnitOfMasure_GRec.SetRange(Code, 'INNER', 'M-PACK');
                    if ItemUnitOfMasure_GRec.FINDSET then
                        REPEAT
                            if ItemUnitOfMasure_GRec.Code = 'INNER' then
                                INNER_GVar := ItemUnitOfMasure_GRec."Qty. per Unit of Measure";
                            if ItemUnitOfMasure_GRec.Code = 'M-PACK' then
                                MPACK_GVar := ItemUnitOfMasure_GRec."Qty. per Unit of Measure";
                        UNTIL ItemUnitOfMasure_GRec.NEXT = 0;

                    Item_LRec.SetRange("No.", "Purchase Line"."No.");
                    if Item_LRec.FindFirst() then begin
                        RE.RESET;
                        RE.SETRANGE("Source Type", 39);
                        RE.SETRANGE("Source Subtype", 1);
                        RE.SETRANGE("Source ID", "Purchase Line"."Document No.");
                        RE.SETRANGE("Item No.", Item_LRec."No.");
                        RE.SETRANGE("Source Ref. No.", "Purchase Line"."Line No.");
                        IF RE.FINDFIRST then begin
                            LotNo := RE."Lot No.";
                        end
                        else begin
                            ITS.RESET;
                            ITS.SETRANGE("Source Type", 39);
                            ITS.SETRANGE("Source Subtype", 1);
                            ITS.SETRANGE("Source ID", "Purchase Line"."Document No.");
                            ITS.SETRANGE("Item No.", Item_LRec."No.");
                            ITS.SETRANGE("Source Ref. No.", "Purchase Line"."Line No.");
                            IF ITS.FINDFIRST then begin
                                LotNo := ITS."Lot No.";
                            end
                        end;
                    end;
            ...
            begin
                ...
                //  CurrReport.Language := Language.GetLanguageIdOrDefault("Language Code");   //mbr test
    ...
    var
        ...
        MPACK_GVar: Decimal;
        SRNo: Integer;
        ItemUnitOfMasure_GRec: Record "Item Unit of Measure";
        LotNo: Code[50];
        Lot_Desc: Text[100];
...
#pragma warning disable AL0755
        // Language: Codeunit Language;  //mbr 5/14/25 - disabled
...
#pragma warning restore AL0667
    ...
    begin
        ...
        PurchHeaderBuyFromCustom(BuyFromAddr, PurchaseHeader);
        if PurchaseHeader."Buy-from Vendor No." <> PurchaseHeader."Pay-to Vendor No." then
            FormatAddr.PurchHeaderPayTo(VendAddr, PurchaseHeader);
        FormatAddr.PurchHeaderShipTo(ShipToAddr, PurchaseHeader);
        IF STRLEN(ShipToAddr[2]) = 0 then begin
            ShipToAddr[1] := CompanyAddr[1];
            ShipToAddr[2] := CompanyAddr[2];
            ShipToAddr[3] := CompanyAddr[3];
            ShipToAddr[4] := CompanyAddr[4];
            ShipToAddr[5] := CompanyAddr[5];
            ShipToAddr[6] := CompanyAddr[6];
            ShipToAddr[7] := CompanyAddr[7];
            ShipToAddr[8] := CompanyAddr[8];
    ...
    local procedure PurchHeaderBuyFromCustom(var AddrArray: array[8] of Text[100]; var PurchHeader: Record "Purchase Header")
    begin
        // Corrected: Remove extra empty string parameter
        FormatAddr.FormatAddr(
            AddrArray,
            PurchHeader."Buy-from Vendor Name",
            '', // Name2
            '', //Contact Name
            PurchHeader."Buy-from Address",
            PurchHeader."Buy-from Address 2",
            PurchHeader."Buy-from City",
            PurchHeader."Buy-from Post Code",
            PurchHeader."Buy-from County",
