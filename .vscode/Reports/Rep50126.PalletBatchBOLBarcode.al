report 50126 PalletBatchBOLBarcode
{
    Caption = 'Pallet Batch Package Label';
    UsageCategory = Administration;
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = './.vscode/ReportLayout/PalletBatchBOLBarcode.rdl';
    ...
    dataset
    {
        dataitem(TmpTable; TmpTable)
        {
            ...
            dataitem(CustomSalesLine; CartonInformation)
            {
                ...
                DataItemTableView = SORTING("Serial No.", "Line No.", "Document Type", "Document No.", "DocumentLine No.");
                ...
                column(CasePackQty; CustomSalesLine."Package Quantity")
                {
                }
                ...
                column(CartInfoCount; CartInfoCount)
                {
                ...
                begin
                    if CustomSalesLine."Document Type" = CustomSalesLine."Document Type"::Order then begin
                        if STRLEN(CustomSalesLine."Serial No.") = 0 then
                            CurrReport.Skip();

                        if ChkSSCC = CustomSalesLine."Serial No." then
                            CurrReport.Skip();

                        ChkSSCC := CustomSalesLine."Serial No.";
                        ...
                        //counts cart info with same sscc
                        gtCartonInfo.Reset();
                        gtCartonInfo.SetRange("Document Type", CustomSalesLine."Document Type");
                        gtCartonInfo.setrange("Document No.", CustomSalesLine."Document No.");
                        gtCartonInfo.SetRange("Serial No.", CustomSalesLine."Serial No.");
                        if gtCartonInfo.FindSet() then
                            CartInfoCount := gtCartonInfo.Count;
                    end
                    else
                        CurrReport.Skip();
                end;
